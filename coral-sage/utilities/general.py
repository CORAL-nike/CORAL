...
def sum_of_squares_friendly(n):
    ...
    bad_prime_prod = (
        9758835872005600424541432018720696484164386911251445952809873289484557900064682994157437552186699082331048989
    )
...
def is_inert(ω, p):
    ...
    def discriminant(ω):
        ...
        if Trd not in ZZ or Nrd not in ZZ:
            raise ValueError("The element ω must be integral")
...
def batch_cofactor_mul_generic(G_list, pis, group_action, lower, upper):
    ...
    if lower > upper:
        raise ValueError("Wrong input to cofactor_multiples()")
