...
from .divisor_arithmetic import affine_add, affine_dbl_iter
...
class RichelotCorr:
    ...
    * 1 has coefficient Gred1(xa) Gred1(xb) h1(x)^2 (x-xa)(x-xb)
    ...
    def map(self, D):
        ...
        Py1 = (z0 + z0 + z3 + z3 + s * (z1 + z2)) * self.x - (p * (z1 + z1 - z2 - z2) + s * (z0 + s * z2 + z3))
        Py1 *= self.H1
...
def _check_maximally_isotropic(P, Q, R, S, a):
    ...
    if ePQ * eRS != 1:
        print("the kernel is not maximally isotropic, e(P,Q)*e(R,S) != 1")
        return False, [None] * 4
...
def compute_richelot_chain(ker_Phi, b, N_constant, strategy):
    ...
    chain, domain = split_richelot_chain(glue_P1, glue_Q1, glue_P2, glue_Q2, b, N_constant, strategy)
    if chain is None:
