...
class ThetaStructure:
    ...
    def coerce_to(self, ring):
        self._base_ring = ring
        self._null_point = self._point(self, [ring(c) for c in self.coords()])
        self._precomputation = None if self._precomputation is None else [ring(el) for el in self._precomputation]
...
class ThetaPoint:
    ...
    def coerce_to(self, ring):
        self._parent.coerce_to(ring)
        self._coords = [ring(c) for c in self._coords]
        self._hadamard = None if self._hadamard is None else [ring(c) for c in self._hadamard]
        self._squared_theta = None if self._squared_theta is None else [ring(c) for c in self._squared_theta]
