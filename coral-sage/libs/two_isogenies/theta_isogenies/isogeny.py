...
class ThetaIsogeny(Morphism):
    ...
    def _compute_codomain(self, T1, T2):
        ...
        """
        if self._hadamard[0]:
            xA, xB, _, _ = ThetaPoint.to_squared_theta(*ThetaPoint.to_hadamard(*T1.coords()))
            zA, tB, zC, tD = ThetaPoint.to_squared_theta(*ThetaPoint.to_hadamard(*T2.coords()))
        else:
        ...
        if not self._hadamard[0] and self._domain._precomputation:
            ...
            # Batch invert denominators
            xA_inv, zA_inv, tB_inv, xB_inv, zC_inv, tD_inv = batched_inversion(xA, zA, tB, xB, zC, tD)
    ...
    def __call__(self, P):
        ...
        if self._hadamard[0]:
            xx, yy, zz, tt = ThetaPoint.to_squared_theta(*ThetaPoint.to_hadamard(*P.coords()))
        else:
