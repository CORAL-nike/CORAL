...
class ThetaIsogeny4(ThetaIsogeny):
    ...
    def _compute_codomain(self, T1, T2):
        ...
        """
        if self._hadamard[0]:
            AA, BB, CC, DD = ThetaPoint.to_squared_theta(*ThetaPoint.to_hadamard(*self._domain.coords()))
            xAB, _, xCD, _ = ThetaPoint.to_squared_theta(*ThetaPoint.to_hadamard(*T1.coords()))
        else:
