...
class EllipticProductIsogeny(Morphism):
    ...
    def __init__(self, kernel, n, strategy=None, zeta=None, coerce=None):
        self.n = n
        ...
        self.coerce = coerce
        self.Fp = GF(kernel[0][0][0].parent().characteristic())
        self.Fp2 = kernel[0][0][0].parent()
    ...
    def isogeny_chain(self, kernel):
        ...
        for k in range(self.n):
            ...
            if k == 0 and self.coerce is not None:
                # These values have been precomputed in accordance to the strategy
                kernel_elements.extend(self.coerce)
                ker = kernel_elements[-1]

            while prev != (self.n - 1 - k):
                level.append(self.strategy[strat_idx])

                if not (k == 0 and self.coerce is not None):
            ...
            if k == 0:
                ...
                if __debug__:
                    assert not self.coerce or all(
                        [c.parent() == self.Fp for c in Tp1.coords() + Tp2.coords() + Th.coords()]
                    )
            ...
            if k == 0 and self.coerce:
                for T1, T2 in kernel_elements:
                    T1.coerce_to(self.Fp)
                    T2.coerce_to(self.Fp)
