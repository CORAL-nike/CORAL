...
from ..isogeny_diamond import DIAMONDS, generate_splitting_kernel
...
def check_result(E0, EA, EB, B, Phi):
    ...
    K_img = Phi(L1)[index]
    verbose_print(f"Computing an image took: {time.process_time() - t0:.5f}", verbose=verbose)
...
def test_SIDH_attack(test_index=-1, verbose=False):
    ...
    verbose_print(f"(2,2)-sqrt-chain took: {time.process_time() - t0:.5f}s", verbose=verbose)
