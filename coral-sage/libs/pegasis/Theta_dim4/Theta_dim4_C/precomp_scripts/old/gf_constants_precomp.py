...
def compute_cofactor_for_2f(p):
    ...
    bit_length = ceil(log(N * 1.0) / log(2.0))
...
def create_gf_constants_cfile(p, num=20, primename=None, path="../src/precomp"):
    ...
    NWORDS_FIELD = ceil(log(p * 1.0) / (64 * log(2.0)))
    for x in nqr_table:
        str_nqr_table += " {{ "
        str_xre = hex(x[0])
    ...
    for i in range(NWORDS_P_COFACTOR_FOR_2F):
        str_p_cofactor_for_2f += (
            "0x"
            + str_hex_cofactor[2 + 16 * (NWORDS_P_COFACTOR_FOR_2F - 1 - i) : 2 + 16 * (NWORDS_P_COFACTOR_FOR_2F - i)]
        )
