...
class KaniClapotiIsog(IsogenyChainDim4):
    ...
    def __init__(self, points, integers, strategy=None):
        ...
        B_Kpp = [
            TuplePoint(xuNbk * Pu, yuNbk * Pu, xv * Pv, yv * Pv),
    ...
    def extract_montgomery_curve(self):
        ...
        if (self.iso_type == "Hadamard" and not (Acan + 2).is_square()) or (
            self.iso_type == "Id" and (Acan + 2).is_square()
        ):
            Acan = -Acan
    ...
    def find_Fp_rational_theta_struct_Ea(self):
        ...
        if Pa[0] == 0 or Pa[1] == 0:
            ...
            raise ValueError(
                "A rational theta point should be mapped to (0:1) or (1:0) after change of theta coordinates on Ea."
            )
    ...
    def evaluate(self, P):
        ...
        FP = TuplePoint(
            [
                theta_point_to_montgomery_point(self.Ea, self.theta_null_Ea, self.iso_Ea(FP[0]), self.twist),
