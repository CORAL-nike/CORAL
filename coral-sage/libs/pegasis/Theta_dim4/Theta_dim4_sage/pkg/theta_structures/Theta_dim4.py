...
from ..theta_structures.theta_helpers_dim4 import (
    act_point,
    batch_inversion,
    hadamard,
...
class ProductThetaStructureDim1To4(ThetaStructureDim4):
    def __init__(self, *args):
        ...
        if len(args) == 4:
            ...
            raise ValueError(
                f"4 or 8 arguments expected but {len(args)} were given.\nYou should enter a list of 4 elliptic curves or ThetaStructureDim1\nor a list of 4 elliptic curves with a 4-torsion basis for each of them."
            )

        self._theta_structures = theta_structures

        null_point = product_theta_point_dim4(
            theta_structures[0].zero().coords(),
            theta_structures[1].zero().coords(),
            theta_structures[2].zero().coords(),
            theta_structures[3].zero().coords(),
        )
    ...
    def product_theta_point(self, theta_points):
        return self._point(
            self,
            product_theta_point_dim4(
                theta_points[0].coords(), theta_points[1].coords(), theta_points[2].coords(), theta_points[3].coords()
            ),
        )
...
class ThetaPointDim4:
    ...
    def double(self, apply_arith_base_change=True):
        ...
        if apply_arith_base_change and self.parent()._arith_base_change:
            P = self.parent().apply_arith_base_change(
                P
            )  # apply_base_change_theta_dim4(self.parent()._arith_base_change_matrix,P)
        ...
        if apply_arith_base_change and self.parent()._arith_base_change:
            theta_2P = self.parent().apply_arith_base_change_inverse(
                theta_2P
            )  # apply_base_change_theta_dim4(self.parent()._arith_base_change_matrix_inv,theta_2P)
