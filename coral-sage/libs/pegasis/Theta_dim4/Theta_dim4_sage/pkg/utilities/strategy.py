...
from sage.all import cached_function, log
...
def optimised_strategies_with_first_eval_new(n, mul_c=1, first_eval_c=1):
    ...
    for i in range(2, n + 1):
        ...
        b, cost = min(
            ((b, C_right[i - b] + C_right[b] + b * mul_c + (i - b) * eval_c) for b in range(1, i)), key=lambda t: t[1]
        )
    ...
    for i in range(3, n + 1):
        ...
        b, cost = min(
            ((b, C_left[i - b] + C_right[b] + b * mul_c + (i - 1 - b) * eval_c + first_eval_c) for b in range(1, i)),
            key=lambda t: t[1],
        )
...
def optimised_strategies_with_first_eval(n, mul_c=1, first_eval_c=1):
    ...
    for i in range(3, n + 1):
        ...
        b, cost = min(
            (
                (b, C_left[i - b] + C_right[b] + b * mul_c + (i - 1 - b) * eval_c + first_eval_c)
    ...
    for d in range(1, m + 1):
        ...
        if d == 1:
            for i in range(3, n - m + d):
                b, cost = min(
                    (
                        (b, C_middle[i - b] + trans_C_right[1][b] + b * mul_c + (i - b) * eval_c)
                        for b in [1] + list(range(3, i))
                    ),
                    key=lambda t: t[1],
                )
...
def precompute_strategy_with_first_eval_and_splitting(e, m, M=1, S=0.8, I=100):
    ...
    return optimised_strategy_with_first_eval_and_splitting(
        n, m, mul_c=mul_c / eval_c, first_eval_c=first_eval_c / eval_c
    )
