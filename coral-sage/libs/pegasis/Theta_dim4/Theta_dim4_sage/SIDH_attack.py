...
from pkg.montgomery_isogenies.isogenies_x_only import (
...
def SIDH_key_exchange(params):
    ...
    t2 = time()
    print(f"Parameter generation time: {t2 - t1} s\n")
...
def SIDH_key_recovery_attack(params, pub_params, pubA, pubB, EAB=None):
    ...
    t4 = time()
    print(f"Evaluation of Bob's isogeny phiB: {t4 - t3} s")
    ...
    t5 = time()
    print(f"Discrete log to find ker(phiB): {t5 - t4} s")
    ...
    for s in sb_candidates:
        ...
        if EC.j_invariant() == EB.j_invariant():
            ...
            break
    t6 = time()
    print(f"Time to recompute phiB: {t6 - t5} s")
    ...
    t7 = time()
    print(f"Computation of shared secret: {t7 - t6} s")
    if EAB != None:
