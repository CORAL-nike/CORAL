...
from pkg.montgomery_isogenies.isogenies_x_only import evaluate_isogeny_x_only, isogeny_from_scalar_x_only
from pkg.parameters.parameter_generation import find_param, find_param_gen, read_params, save_params
from pkg.theta_structures.Tuple_point import TuplePoint
from pkg.utilities.strategy import (
    precompute_strategy_with_first_eval,
)  # , precompute_strategy_with_first_eval_and_splitting
from pkg.utilities.supersingular import torsion_basis
...
def lookup_and_display_params(index, l_B):
    ...
    print(f" - Index in the list of parameters = {index}")
    print(f" - Prime characteristic p = {f} * 2**{f_A} * {l_B}**{f_B} - 1")
    print(f" - Degree of the embedded isogeny sigma q = {l_B}**{e_B}")
    print(f" - a1 = {a1}")
    print(f" - a2 = {a2}")
    print(f" - m = max(v_2(a1),v_2(a2)) = {m}")
    print(f" - Length of the 4-dimensional 2-isogeny = {e_A}")
...
def display_all_params(l_B, index):
    if l_B == None:
        for l in d_L_params:
            print("===========================================")
            print(f"All parameters with second prime l_B={l}.")
            print("===========================================\n")
        ...
        raise ValueError(f"No parameter with prime l_B={l_B} has been generated.")
    elif index == None:
        l = int(l_B)
        print("===========================================")
        print(f"All parameters with second prime l_B={l}.")
...
def add_params(l_B, e_A):
    ...
    print("===========================================")
    print(f"New parameters with second prime l_B={l_B}.")
    print("===========================================\n")

    for i in range(len(L_params)):
        if l_B == 3:
            e_A, e_B, a1, a2, f, f_A, f_B, p = L_params[i]
            print(f" - Index in the list of parameters = {i + len(L_params_prev)}")
            print(f" - Prime characteristic p = {f} * 2**{f_A} * {l_B}**{f_B} - 1")
            print(f" - Degree of the embedded isogeny sigma q = {l_B}**{e_B}")
            print(f" - a1 = {a1}")
            print(f" - a2 = {a2}")
...
def test_kani_endomorphism(l_B, e_A, e_B, a1, a2, f, f_A, f_B, p, m=None, primality_check=True):
    ...
    print(f" - a2 = {a2}")
    print(f" - m = max(v_2(a1),v_2(a2)) = {m}")
    print(f" - Length of the dimension 4 2-isogeny = {e_A}")
    ...
    print(
        f"Is evaluation correct?\n{(FT[0][0] == (a1 * P1)[0]) & (FT[1][0] == (-a2 * P1)[0]) & (FT[2][0] == (-R2)[0]) & (FT[3] == E2(0))}"
    )

    print(f"Time evaluation: {t7 - t6} s\n")
...
def test_kani_endomorphism_half(l_B, e_A, e_B, a1, a2, f, f_A, f_B, p, m=None, primality_check=True):
    ...
    print(f" - m = max(v_2(a1),v_2(a2)) = {m}")
    print(f" - Length of the dimension 4 2-isogeny = {e_A}")
    print(f" - Used available torsion = 2**{ceil(e_A / 2) + 2}")
    ...
    t5 = time()
    print(f"Computation of strategies: {t5 - t4} s")
        ...
        if args.KaniEndo:
            if args.l_B == None:
                for l_B in d_L_params:
                    print("===========================================================")
                    print(
                        f"Testing Kani endomorphism computation (class KaniEndo) when\nthe embedded isogeny has degree deg(sigma) = {l_B}**e_B."
                    )
                    print("===========================================================\n")
        ...
        if args.KaniEndoHalf:
            if args.l_B == None:
                for l_B in d_L_params:
                    print("===============================================================================")
                    print(
                        f"Testing Kani endomorphism with half the torsion computation (class KaniEndoHalf)\nwhen the embedded isogeny has degree deg(sigma) = {l_B}**e_B."
                    )
                    print("===============================================================================\n")
