...
from .xonly import MontgomeryA, xPoint
...
def TwoTorsBasis(E, e):
    ...
    """
    E_fp = E.change_ring(GF(E.base_field().characteristic()))
    logger.debug("     > Finding TwoTorsionBasis")
    tstart = time()

    T0, Tm1, T1 = find_Ts(E_fp)
    ...
    assert (E_fp.base_field().characteristic() + 1) % 2 ** (e + 1) == 0
    cofac = (E_fp.base_field().characteristic() + 1) / 2 ** (e + 1)
    P = P.xMUL(cofac)
    Q = Q.xMUL(cofac)
