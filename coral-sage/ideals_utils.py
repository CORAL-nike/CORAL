...
def coords_anti_diagonals(cf_b):
    """
    Yield (x, y) with x >= 0, y >= 0, in increasing x+y order:
    (1,0), (-1,0), (0,1), (1,1), (0,2), (2,1), (3,0), ...
    """
    for s in range(1, 2 * cf_b):          # sums s = x + y
        x_max = min(s, cf_b - 1)
        x_min = max(0, s - (cf_b - 1))
...
def short_vectors(L, B, cf_b=15):
    ...
    for cff in itt.product(range(-cf_b, cf_b), range(0, cf_b)):
        ...
        nv = _norm(v)
        if nv < B and nv != 0 and nv not in sv:
            sv[nv] = v
    sv = [(k, sv[k]) for k in sv]
    sv.sort()
    return sv

def short_vectors_plus(L, B, cf_b=15):
    """
    Enumerate vectors in L with norm bounded by B.
    Tries linear combinations with coefficients up to cf_b
    """
    sv = {}
    assert len(L) == 2  # Avoiding sign repeats
    for cff in coords_anti_diagonals(cf_b):
        v = [0 for _ in range(len(L[0]))]
        for i in range(len(L)):
            v = _add(v, _mul(L[i], cff[i]))
        nv = _norm(v)
